    ...
    if ($tlang['tlang_available'] == 1) {?>
        <li class="<?php echo ($tlang['tlang_subcategories'] > 0) ? 'is-child' : ''; ?>">
            <?php if (count($tlang['children']) > 0) { ?>
                <span class="trigger accordion-header"><?php echo $tlang['tlang_name']; ?></span>
                <?php
                ...
                ?>
                <?php $this->includeTemplate('teacher/languages.php', ['languages' => $tlang['children'], 'values' => $values]); ?>
            <?php } else { ?>
                <label class="accordion-trigger">
                    <input type="checkbox" value="<?php echo $key; ?>" name="teach_lang_id[]" <?php echo in_array($key, $values) ? 'checked' : ''; ?> >
